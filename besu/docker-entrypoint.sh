...
if [ "${ARCHIVE_NODE}" = "true" ]; then
  ...
  if [ -f "${__timestamp_file}" ]; then
    ...
    if (( __diff >= 172800 )); then  # 48 * 60 * 60 - 48 hours have passed
      ...
      __prune+=" --Xplugin-rocksdb-blockchain-blob-garbage-collection-enabled=true --Xplugin-rocksdb-blob-garbage-collection-age-cutoff=0.5 --Xplugin-rocksdb-blob-garbage-collection-force-threshold=0.1"
    fi
  fi
else
  __prune=""
...
# shellcheck disable=SC2086
  exec /opt/besu/bin/besu ${__datadir} ${__network} storage x-prune-pre-merge-blocks
